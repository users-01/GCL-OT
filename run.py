...
logging.basicConfig(level=logging.INFO,  # 设置日志记录级别
    ...
    filename='main_cora_otclout_add.log', filemode='a'  # 'w' 表示写模式，每次运行都会覆盖文件，'a' 表示追加模式
    )
...
if __name__ == "__main__":
    ...
    '''
    python run.py --data_name cornell --gnn_name gcn --freeze_layers_count 3 --weight_loss_graph 0.01 --weight_loss_text 1.0 --fuse_way add
    python run.py --data_name cornell --gnn_name gcn --freeze_layers_count 3 --weight_loss_graph 0.01 --weight_loss_text 1.0 --fuse_way concat
    python run.py --data_name cornell --gnn_name gcn --freeze_layers_count 6 --weight_loss_graph 0.01 --weight_loss_text 1.0 --fuse_way add
    python run.py --data_name cornell --gnn_name gcn --freeze_layers_count 1 --weight_loss_graph 0.01 --weight_loss_text 1.0 --fuse_way add
    python run.py --data_name cornell --gnn_name gcn --freeze_layers_count 0 --weight_loss_graph 0.01 --weight_loss_text 1.0 --fuse_way add
    '''
    torch.rand(1).cuda()
    print("-----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------")
